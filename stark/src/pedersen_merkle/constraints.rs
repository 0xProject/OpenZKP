...
pub fn get_pedersen_merkle_constraints(public_input: &PublicInput) -> Vec<Constraint> {
    ...
    let no_rows = SparsePolynomial::new(&[FieldElement::ONE]);
    let first_row = SparsePolynomial::new(&[-&FieldElement::ONE, FieldElement::ONE]);
    ...
    let hash_start_rows =
        SparsePolynomial::new(&[(path_length, FieldElement::ONE), (0, -&FieldElement::ONE)]);
    let every_row =
        SparsePolynomial::new(&[(trace_length, FieldElement::ONE), (0, -&FieldElement::ONE)]);
    ...
    vec![
        ...
        Constraint {
            ...
            base:        Box::new(move |tp| {
                (tp[0].clone() - SparsePolynomial::new(leaf.clone()))
                    * (tp[4].clone() - SparsePolynomial::new(leaf.clone()))
            }),
            numerator:   no_rows.clone(),
            denominator: first_row.clone(),
        },
        Constraint {
            base:        Box::new(move |tp| SparsePolynomial::new(root.clone()) - tp[6].clone()),
            numerator:   no_rows.clone(),
        ...
        Constraint {
            base:        Box::new(move |tp| {
                tp[6].clone() - SparsePolynomial::new(shift_point_x.clone())
            }),
            numerator:   no_rows.clone(),
        ...
        Constraint {
            base:        Box::new(move |tp| {
                tp[7].clone() - SparsePolynomial::new(shift_point_y.clone())
            }),
            numerator:   no_rows.clone(),
            denominator: hash_start_rows.clone(),
        },
        Constraint {
            base:        Box::new(|tp| {
                ...
                left_bit.clone() * (left_bit - SparsePolynomial::new(FieldElement::ONE))
            }),
        ...
        Constraint {
            base:        Box::new(move |tp| {
                let left_bit = get_left_bit(tp, g);
                (SparsePolynomial::new(FieldElement::ONE) - left_bit)
        ...
        Constraint {
            base:        Box::new(|tp| {
                ...
                right_bit.clone() * (right_bit - SparsePolynomial::new(FieldElement::ONE))
            }),
            numerator:   hash_end_rows.clone(),
            denominator: every_row.clone(),
        },
        ...
        Constraint {
            base:        Box::new(move |tp| {
                let right_bit = get_right_bit(tp, g);
                (SparsePolynomial::new(FieldElement::ONE) - right_bit)
