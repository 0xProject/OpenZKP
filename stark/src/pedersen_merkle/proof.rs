...
mod tests {
    ...
    fn pedersen_merkle_proof() {
        ...
        let constraint_polynomial =
            get_constraint_polynomial(&trace_polynomials, &constraints, &get_coefficients(), 2);

        let constraint_lde =
            evalute_polynomial_on_domain(&constraint_polynomial, proof_parameters.blowup);

        let extended_constraint_table = calculate_low_degree_extensions(&constraint_polynomials, proof_parameters.blowup);
    }
