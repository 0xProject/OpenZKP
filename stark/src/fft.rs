...
mod tests {
    ...
    fn fft_eight_element_test() {
        ...
        let expected: Vec<FieldElement> = (0..8u64)
            .map(|i| Polynomial::new(&v).evaluate(&eighth_root_of_unity.pow(U256::from(i))))
            .collect();
    ...
    fn fft_test() {
        ...
        for (i, x) in fft(&vector).into_iter().enumerate() {
            assert_eq!(
                x,
                Polynomial::new(&vector).evaluate(&root.pow(U256::from(i as u64)))
            );
        }
