...
pub fn fft_cofactor(
    ...
    fft_pure(root, vector)
}

pub fn ifft(root: FieldElement, vector: Vec<FieldElement>, cofactor: Option<FieldElement>) -> Vec<FieldElement> {
    match cofactor{
        None => {return ifft_pure(root, vector);},
        Some(x) => {return ifft_cofactor(root, vector, x);},
    }
}

pub fn ifft_pure(root: FieldElement, vector: Vec<FieldElement>) -> Vec<FieldElement> {
...
pub fn ifft_cofactor(root: FieldElement, vector: Vec<FieldElement>, cofactor: FieldElement) -> Vec<FieldElement> {
    let mut r = fft_pure((&root).inv().unwrap(), vector);
    let len_el = FieldElement::from(U256::from((&r).len() as u64));
    let s = len_el.inv().unwrap();
...
mod tests {
    ...
    fn basic_fft_test() {
        ...
        let mut vector = vec![2u64,1,3,4].into_iter().map(|n| FieldElement::from(U256::from(n))).collect();
    ...
    fn fft_test() {
        ...
        let mut res = fft(root.clone(), vector.clone(), None);

        assert_eq!(U256::from(&res[0]), u256h!("06a1b7c038205cb38aaeea38662ae2259a19c14a7519bd522543f72dc7fa74b2"));
        assert_eq!(U256::from(&res[1]), u256h!("017884f169b20153de79a9c642d4e3259263f2e7ac5f85f5a8191f28d8f14544"));
        assert_eq!(U256::from(&res[2]), u256h!("03112a352e474819d491a13b700a07161eee580ff40098df978fa19f39b4fd2d"));
        assert_eq!(U256::from(&res[3]), u256h!("011606a821f418d13914c72b424141c5b88bdb184b0b5a55fc537587346c78a2"));
        assert_eq!(U256::from(&res[4]), u256h!("00dc2519322c102b8ad3628106a3ebef7c39f85215203bfc820c7a04a9645419"));
        assert_eq!(U256::from(&res[5]), u256h!("01df6a70d033d89376c96c45ce8dbbe4eeedce2d32636c29d3cb87b9e2074d00"));
        assert_eq!(U256::from(&res[6]), u256h!("00ee6a5e89e9307e64789e1a71c42105de12bfa104e32c5a381fe5c2697ffeec"));
        assert_eq!(U256::from(&res[7]), u256h!("048bad0760f8b52ee4f9a46964bcf1ba9439a9467b2576176b1319cec9f12db0"));

        res = fft(root, vector, Some(cofactor));

        assert_eq!(U256::from(&res[0]), u256h!("05d817ee1af8beff1880aad163a9912704d66e0c717a670c52db93da5ea34455"));
        assert_eq!(U256::from(&res[1]), u256h!("0631b16aceb1ee5711066df1ffafd9f5f451b0dc44c86e90005bc78e8bb4f861"));
        assert_eq!(U256::from(&res[2]), u256h!("01a30c98c149179cd16059ba201b99cf629d3e04844a50936006a185a67ad354"));
        assert_eq!(U256::from(&res[3]), u256h!("07a17b9035ff1ffd1f9e0bc52982effcd957bc07230830c10e51e906ed092f9e"));
        assert_eq!(U256::from(&res[4]), u256h!("01381787eccc6c77b0c5dff0b4b66dc0bb7d911bd705baf85f62001976e6ff27"));
        assert_eq!(U256::from(&res[5]), u256h!("009defa0822d287ce55035bb705319eb34e78180157e5297e6a46df9af8ef042"));
        assert_eq!(U256::from(&res[6]), u256h!("020b8317360c61abbc0bdce513eb42295402eb5dde3d13abfc0325f277f507bc"));
        assert_eq!(U256::from(&res[7]), u256h!("034738bd5956b1df55369cdc211109fd67e6ffd2ffbb08e856b1b4d1b1a2c6ae"));
    ...
    fn ifft_test() {
        ...
        let mut res = fft(root.clone(), vector.clone(), None);
        let mut res_inv = ifft(root.clone(), res, None);

        for index in 0..8{
            assert_eq!(vector[index], res_inv[index]);
        }

        res = fft(root.clone(), vector.clone(), Some(cofactor.clone()));
        res_inv = ifft(root, res, Some(cofactor));
