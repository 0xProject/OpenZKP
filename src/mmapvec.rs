...
use std::mem::size_of;
use std::ops::{Deref, DerefMut, Drop, Index, IndexMut, Range, RangeFrom, RangeTo, RangeFull};
use std::path::PathBuf;
use std::ptr;
use std::slice;
use memmap::{MmapMut, MmapOptions};

/// A subset of Vec<T> features based on a mmap-ed file. Application should take care of
/// locking so `MVec`s backed by a same file would either have multiple readers, or have
/// a unique reader.
...
impl<T: Copy> MmapVec<T> {
    ...
    pub fn from_path<P: Borrow<PathBuf>>(path: P) -> Result<Self> {
    ...
    pub fn reserve(&mut self, additional: usize) {
        ...
        if required_cap > self.cap {
            // Preallocate more space to reduce disk fragments. Neither too big nor too small.
    ...
    pub fn sync(&self) -> Result<()> { Ok(self.mmap.flush()?) }
}
...
macro_rules! impl_index {
    ($I: ty, $T: tt, $O: tt) => {
        impl<$T: Copy> Index<$I> for MmapVec<T> {
            type Output = $O;
            #[inline]
            fn index(&self, index: $I) -> &$O { self.deref().index(index) }
        }

        impl<$T: Copy> IndexMut<$I> for MmapVec<T> {
            #[inline]
            fn index_mut(&mut self, index: $I) -> &mut $O { self.deref_mut().index_mut(index) }
        }
    }
}
...
mod tests {
    ...
    fn check_with_vec<T: Copy + Eq + Default + Debug>(v: &Vec<T>) -> Result<()> {
