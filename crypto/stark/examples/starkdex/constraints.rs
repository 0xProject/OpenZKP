...
pub fn constraints(claim: &Claim, parameters: &Parameters) -> Vec<RationalExpression> {
    ...
    (Trace(0, row_expression_0) - initial_root) / (X - 1.into()), // initial_root
    (Trace(4, row_expression_1) - final_root) / (X - trace_generator.pow(65536 * (trace_length / 65536 - 1))), // final_root
    (Trace(4, row_expression_0) - Trace(0, row_expression_2)) * (X - trace_generator.pow(65536 * (trace_length / 65536 - 1) + 49152)) / (X.pow(trace_length / 16384) - 1.into()), // copy_merkle_roots
    (is_modification.clone() * (Trace(4, row_expression_0) - Trace(4, row_expression_1))) / (X.pow(trace_length / 65536) - 1.into()), // copy_merkle_roots_modification
    ]
}
