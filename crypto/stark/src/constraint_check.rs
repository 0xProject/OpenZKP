...
use crate::trace_table::*;
use std::convert::TryInto;
use zkp_primefield::FieldElement;

pub(crate) fn check_constraints(constraints: &Constraints, table: &TraceTable) -> Result<(), (usize, usize)> {
...
pub(crate) fn check_specific_constraint(constraints: &Constraints, table: &TraceTable, row: usize, which_constraint: usize) -> bool {
    let trace_generator = FieldElement::root(table.num_rows()).unwrap();
    let x;
    if row == 0 {
        x = FieldElement::ONE;
    ...
    constraints.expressions()[which_constraint].check(&x, &trace).1 
}
...
mod tests { 
    ...
    fn specific_constraint_checker() {
        ...
        constraints.num_queries = 20;
        constraints.fri_layout = vec![3, 2];
        assert_eq!(check_specific_constraint(&constraints, &trace, 1000, 1), true);
