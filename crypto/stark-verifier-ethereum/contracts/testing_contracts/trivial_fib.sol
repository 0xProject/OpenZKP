...
contract TrivialFib is ConstraintSystem {
    ...
    function calculate_commited_polynomial_points(ProofTypes.StarkProof calldata proof, ProofTypes.ProofParameters calldata params, uint64[] calldata queries, bytes32 oods_point, bytes32[] calldata constraint_coeffiencts) external view override returns(bytes32[] memory, bytes32[] memory) {
        bytes32[20] memory data_1 = [
        ...
        bytes32[] memory result_1 = new bytes32[](20);
        bytes32[] memory result_2 = new bytes32[](20);
        for (uint i = 0; i < 20; i ++) {
            result_1[i] = data_1[i];
            result_2[i] = data_2[i];
        }
