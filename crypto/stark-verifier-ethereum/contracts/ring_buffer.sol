...
library RingBuffer {
    ...
    function add_to_rear(IndexRingBuffer memory buffer, uint index, bytes32 data) internal pure {
        ...
        if (buffer.is_empty) {
            ...
            uint next = (buffer.back + 1)%buffer.data.length;
    ...
    function remove_front(IndexRingBuffer memory buffer) internal pure returns(uint index, bytes32 data) {
    ...
    function peak_front(IndexRingBuffer memory buffer) internal pure returns(uint index, bytes32 data) {
