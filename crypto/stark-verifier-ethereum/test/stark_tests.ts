...
describe('Stark Testing testing', () => {
    ...
    it('It should have the correct queries after reading', async () => {
        ...
        let converted = return_data.map(x => x.toNumber());
        expect(converted).to.be.deep.equal,([1089, 1175, 1750, 2590, 2747, 4172, 4304, 4443, 4534, 5373, 6525, 7804, 9317, 9568, 11715, 12372, 12762, 14035, 14823, 15894]);
    });
    ...
    it('It correctly identify the number of leading zeros', async () => {
        ...
        for (let i : number = 0;  i < 1; i ++) {
            verifier_contract.leading_zeros_external(random_number.maskn(255-i)).then((result: any) => {
