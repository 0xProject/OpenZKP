...
use openstark::{check_proof, fibonacci, stark_proof, ProofParams};
...
fn proof_make(crit: &mut Criterion) {
    ...
    crit.bench_function("Making an abstracted Fibonacci proof", move |bench| {
        bench.iter(|| {
            black_box(stark_proof(&claim, &witness, &ProofParams {
                blowup:     16,
                pow_bits:   12,
                queries:    20,
...
fn proof_check(crit: &mut Criterion) {
    ...
    crit.bench_function("Checking a fib proof of len 1024", move |bench| {
        bench.iter(|| {
            black_box(check_proof(proof.proof.as_slice(), &claim, &ProofParams {
                blowup:     16,
                pow_bits:   12,
                queries:    20,
                fri_layout: vec![3, 2, 1],
