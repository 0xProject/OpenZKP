...
fn scalar_mult(trace: &mut TraceTable, point: (FieldElement, FieldElement), scalar: &U256, start: usize, offset: usize)  {
...
fn double(point_x: &FieldElement, point_y: &FieldElement) -> (FieldElement, FieldElement) {
...
// Note incorrect when given zero inputs
fn add(x_p: &FieldElement, y_p: &FieldElement, x_q: &FieldElement, y_q: &FieldElement) -> (FieldElement, FieldElement) {
...
mod test {
    ...
    fn scalar_mult_in_table() {
        ...
        scalar_mult(&mut trace, (p.0, p.1), &c, 0, 0);
