...
impl ContextArray {
    fn new() -> Self {
        Self{n_unique_queries: 0, fri_inverses: 0, fri_values: 0, trace_queries: 0, composition_queries: 0, denominators: 0, oods_coefficients: 0, oods_values: 0, composition_oods: 0}
    }
}

// Please note this function assumes a rational expression which is a polynomial over X^len_poly
pub fn autogen_periodic(periodic: &RationalExpression, index: usize, name: &str) -> Result<(), std::io::Error> {
    ...
    match periodic {
        RationalExpression::Polynomial(poly, _) => {
            writeln!(&mut file, "pragma solidity ^0.5.2;
    ...
    function evaluate(uint x) external pure returns (uint y){{
        assembly {{
            ...
            for coef in poly.coefficients().iter().rev() {
                writeln!(&mut file, "                y := addmod(mulmod(x, y, PRIME), 0x{}, PRIME)", U256::from(coef).to_string())?;
            }
            writeln!(&mut file, "        }}
    }}
}}")?;
...
pub fn autogen_constraint_poly(trace_len: usize, public: &[&RationalExpression], constraints: &[RationalExpression], n_rows: usize, n_cols: usize) -> Result<(), std::io::Error>{
    ...
    let mut index = 0;
    for col in periodic_keys.iter() {
        autogen_periodic(col, &format!("periodic{}", index), index)?;
        index += 1;
    }
    ...
    let ctx_layout = autogen_memory_layout(constraints.len(), public.len(), periodic_keys.len(), n_rows, n_cols, trace_keys.len(), max_degree)?;
    autogen_oods(trace_keys.as_slice(), &generator, max_degree, ctx_layout)?;
    let memory_map = setup_call_memory(&mut file, constraints.len(), public, inverse_keys.as_slice(), trace_keys.as_slice(), periodic_keys.as_slice(), adjustment_degrees.as_slice())?;

    let mut coefficient_index = 1 + public.len() + periodic_keys.len();
    for (exp, &degree) in constraints.iter().zip(adjustment_degrees.iter()) {
        writeln!(&mut file, "      {{")?;
        writeln!(&mut file, "        let val := {}", exp.soldity_encode(&memory_map))?;
        writeln!(&mut file, "        res := addmod(res, mulmod(val, add(mload({}), mulmod(mload({}), {}, PRIME)), PRIME),PRIME)", coefficient_index*32, (coefficient_index+1)*32, memory_map.get(&RationalExpression::Exp(RationalExpression::X.into(), degree)).unwrap())?;
        writeln!(&mut file, "      }}")?;
        coefficient_index += 2;
    }
    writeln!(&mut file, "      mstore(0, res)\n      return(0, 0x20)\n  }}\n  }}\n}}")?;
    Ok(())
}

pub fn autogen_memory_layout(num_constraints: usize, n_public_in: usize, n_perodic: usize, n_rows: usize, n_cols: usize, mask_size: usize, degree_bound: usize) -> Result<ContextArray, std::io::Error> {
    ...
    writeln!(&mut file, "pragma solidity ^0.5.0;
    ...
    writeln!(&mut file, "    uint256 constant internal mm_batch_inverse_out =                            {};", index)?;
    index += max_query_size*(n_rows+2);
    writeln!(&mut file, "    uint256 constant internal mm_batch_inverse_in =                            {};", index)?;
    index += max_query_size*(n_rows+2);

    // Formats memory for the constraint calculation call
    writeln!(&mut file, "    uint256 constant internal mm_constraint_poly_args_start =                  {};
    uint256 constant internal mm_oods_point =                                  {};", index, index)?;
    index += 1;

    for i in 0..n_public_in {
        writeln!(&mut file, "    uint256 constant internal mm_public{} =                            {};", i, index)?;
        index +=1;
    }

    for i in 0..n_perodic {
        writeln!(&mut file, "    uint256 constant internal mm_periodic{} =                            {};", i, index)?;
        index +=1;
    }

    writeln!(&mut file, "    uint256 constant internal mm_coefficients =                                {};", index)?;
    index += num_constraints*2;
    ctx.oods_values = index;
    writeln!(&mut file, "    uint256 constant internal mm_oods_values =                                 {};", index)?;
    index += mask_size;
    writeln!(&mut file, "    uint256 constant internal mm_constraint_poly_args_end =                    {};", index)?;
    
    // Formats memory used for oods virtual oracle
    ctx.composition_oods = index;
    writeln!(&mut file, "    uint256 constant internal mm_composition_oods_values =                     {};", index)?;
    index += degree_bound + 1;
    writeln!(&mut file, "    uint256 constant internal mm_oods_eval_points =                            {};", index)?;
    index += max_query_size;
    ctx.oods_coefficients = index;
    writeln!(&mut file, "    uint256 constant internal mm_oods_coefficients =                           {};", index)?;
    index += mask_size + degree_bound;  
    ctx.trace_queries = index;  
    writeln!(&mut file, "    uint256 constant internal mm_trace_query_responses =                       {};", index)?;
    index += max_query_size*n_cols;
    ctx.composition_queries = index;
    writeln!(&mut file, "    uint256 constant internal mm_composition_query_responses =                 {};", index)?;
    index += degree_bound*max_query_size;
    writeln!(&mut file, "    uint256 constant internal mm_trace_generator =                             {};
    uint256 constant internal mm_trace_length =                                {};
    uint256 constant internal mm_context_size =                                {};", index, index+1, index+2)?;
    writeln!(&mut file, "}}")?;

    Ok(ctx)
}

pub fn setup_call_memory(file: &mut File, num_constraints: usize, public_inputs: &[&RationalExpression], inverses: &[&RationalExpression], traces: &[&RationalExpression], periodic: &[&RationalExpression], adjustment_degrees: &[usize]) -> Result<HashMap::<RationalExpression, String>, std::io::Error> {
    ...
    index += num_constraints*2;
    // Note that the trace values must be the last inputs from the contract to make the memory layout defaults work.
    for &exp in traces.iter() {
        memory_lookups.insert(exp.clone(), format!("mload({})", index*32));
        index += 1;
    }
    let in_data_size = index;
    // Here we need to add an output which writelns denominator storage and batch inversion
    ...
    writeln!(file, "pragma solidity ^0.5.2;
...
contract OodsPoly {{
    ...
    writeln!(file, "{}", held)?;
    writeln!(file,"        // Store the values which will be batch inverted")?;
    let mut inverse_position = inverse_start_index;
    for &exp in inverses.iter() {
        match exp {
            RationalExpression::Inv(a) => {writeln!(file, "        mstore({}, {})", index*32, a.soldity_encode(&memory_lookups))?;},
            _ => {panic!("Inverse search returned a non inverse");},
        }
        // Out batch inversion will place the final inverted product before the calculated denom 
...
pub fn autogen_oods(trace: &[&RationalExpression], generator: &FieldElement, degree_bound: usize, ctx: ContextArray) -> Result<(), std::io::Error> {
    ...
    writeln!(&mut file, "  function oods_prepare_inverses(uint256[] memory context) internal pure {{
        ...
        for (uint i = 0; i < context[mm_n_unique_queries]; i ++) {{
            ...
            uint x = fmul(context[i + mm_oods_eval_points], PrimeFieldElement0.generator_val);", U256::from(generator))?;
    
    let mut last_seen_row = std::isize::MAX;
    let mut counter = 0;
    ...
    for element in row_sorted_trace.iter() {
        match element {
            RationalExpression::Trace(_, j) => {
                if *j != last_seen_row {
                    writeln!(&mut file, "            // Preparing denominator for row {}", j)?;
                    writeln!(&mut file, "        context[batch_inverse_chunk*i + {} + mm_batch_inverse_in] = fsub(x, fmul(oods_point, 0x{}));", counter, U256::from(generator.pow(*j)))?;
                    index_to_offset.insert(j, counter*32);
                    counter += 1;
                    last_seen_row = *j;
                }
            },
            _ => panic!("Expected that the trace array was composed of only RationalExpression::Trace"),
        }
    }

    writeln!(&mut file, "        context[batch_inverse_chunk*i + {} + mm_batch_inverse_in] = fsub(x, fpow2(oods_point, {}));", counter, degree_bound)?;
    writeln!(&mut file, "        context[batch_inverse_chunk*i + {} + mm_batch_inverse_in] = context[i + mm_oods_eval_points];", counter + 1)?;
  ...
  writeln!(&mut file, "  function oods_virtual_oracle(uint256[] memory ctx)
  internal {{
  ...
  assembly {{
      ...
      let fri_values_end := add(fri_values,  mul(/*n_unique_queries*/ mload(add(context, {})), 0x20))
      ...
      for {{}} lt(fri_values, fri_values_end) {{fri_values := add(fri_values, 0x20)}} {{
        let res := 0", (ctx.fri_values+1)*32, (ctx.n_unique_queries+1)*32, (ctx.fri_inverses+1)*32, (ctx.trace_queries+1)*32, (ctx.composition_queries+1)*32, (ctx.denominators+1)*32)?;

    let mut seen_col = std::usize::MAX;
    ...
    // We also assume some constraint applies to each col somewhere so that we don't have to index
    for mask_item in trace.iter() {
        match mask_item {
            RationalExpression::Trace(i, j) => {
                if *i != seen_col {
                    if seen_col != std::usize::MAX {
                        writeln!(&mut file, "                }}")?;
                    }

                    writeln!(&mut file, "              // Mask items for column #{}.
                    {{
                    // Read the next element.
                    let column_value := mulmod(mload(add(trace_query_responses, {})), k_montgomery_r_inv, PRIME)", i, i*32)?;
                    seen_col = *i;
                    col_read += 1;
                }
                writeln!(&mut file, "              res := addmod(
                    res,
                    mulmod(mulmod( /* denom for {}th row */ mload(add(denominators_ptr, {})),
                           ...
                           PRIME),
                    PRIME)", j, index_to_offset.get(j).unwrap(), oods_coefficients_ptr, oods_value_ptr)?;
                oods_value_ptr += 32;
    ...
    writeln!(&mut file, "              trace_query_responses := add(trace_query_responses, {})", col_read*32)?;
    writeln!(&mut file, "         // Composition constraints.")?;

    for i in 0..degree_bound {
        writeln!(&mut file, "              {{
            // Read the next element.
            let column_value := mulmod(mload(add(composition_query_responses, {})), k_montgomery_r_inv, PRIME)
            res := addmod(
                res,
                mulmod(mulmod(mload(add(denominators_ptr, {})),
                       ...
                       PRIME),
                PRIME)
            }}", i*32, index_to_offset.len()*32, oods_coefficients_ptr, composition_ptr)?;
            oods_coefficients_ptr += 32;
            composition_ptr += 32;
    }

    writeln!(&mut file, "              // Advance the composition_query_responses by the the amount we've read (0x20 * constraint_degree).
...
}}", degree_bound*32, (index_to_offset.len()+1)*32, (index_to_offset.len() + 2)*32)?;
    Ok(())
}
