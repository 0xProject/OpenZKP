...
impl<T: Trait> ProvideInherent for Module<T> {
	...
	fn check_inherent(call: &Self::Call, data: &InherentData) -> result::Result<(), Self::Error> {
		...
		if t != data1 {
			return Err(InherentError::Other("Posted proof doesn't match proof provided to call".into()))
		}
		...
		if check_proof(
			...
			return Err(InherentError::InvalidProof("Posted proof doesn't pass verification".into()))
		}
	}
}
...
mod tests {
	...
	fn inherent_with_invalid_proof() {
		...
		with_externalities(&mut new_test_ext(), || {
			let _ = BlockProof::dispatch(Call::set(RecordedProof{
		 				proof: actual.proof.clone(),
		 				public: incorrect_public.into() }), RawOrigin::Inherent.into());
		});
	}
}